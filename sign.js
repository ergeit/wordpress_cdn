...
jQuery(document).ready(function(a) {
    ...
    function(b) {
        return a(this).valid() ? (a("p.status", this).show().text(ajax_sign_object.loadingmessage), "login" == a(this).attr("id") ? (action = "ajaxlogin", username = a("form#login #username").val(), password = a("form#login #password").val(), email = "", authcode="",  email_authcode = "",  security = a("form#login #security").val(), remember = "checked" == a("#rememberme").attr("checked") ? a("form#login #rememberme").val() : "", a("form#login .submit").attr("disabled", !0).addClass("disabled").val("登录中...")) : "register" == a(this).attr("id") && (action = "ajaxregister", username = a("#user_name").val(), password = a("#user_pass").val(), email = a("#user_email").val(), authcode = a("#authcode").val(), email_authcode = a("#email_authcode").val(), security = a("#user_security").val(), remember = "", a("form#register .submit").attr("disabled", !0).addClass("disabled").val("注册中...")), _this = a(this), a.ajax({
            type: "POST",
            dataType: "json",
...
jQuery(function() {
 // 发送验证码 注册
   $('.go-captcha_email').on('click', function(event) {
      var _this = $(this);
      var deft = _this.text();
      var email = a("#user_email").val();
    ...
    jQuery("#register").length && jQuery("#register").validate({
        ...
        messages: {
            user_name: {
			 ...
			 authcode: {
                required: "请输入验证码",
                maxlength: a.validator.format("最多输入{0}位")
            },
			email_authcode: {
                required: "请输入验证码",
                maxlength: a.validator.format("最多输入{0}位")
